...
Tech Agency is a Claude Code plugin that provides specialized AI agents, slash-command skills, and coding standards to orchestrate end-to-end software development across Mobile (KMP), Web, and Server platforms.

**Version:** 1.1.1 | **Author:** Zeyad Gasser

<!-- BEGIN:inventory (generated by scripts/generate-inventory.sh — do not edit) -->

| What | Count |
|---|---|
| Agents | 19 |
| Skills (first-party) | 34 |
| Skills (vendored — `android-*`, `kotlin-*`) | 14 |
| Coding standards & rules | 19 |
...
Agents follow these coding standards when writing code. They live in `.claude/rules/`, grouped by domain, and are automatically loaded as project instructions — see the inventory above for current counts.

A consumer repo keeps a **generated mirror** of these under its own `.claude/rules/`, because plugin-supplied rules do not auto-load. See `rules-mirror.md`.
...
| `worktree-first.md` | Every task runs in its own git worktree; base-branch resolution |
| `rules-mirror.md` | The consumer rules mirror — generated, never hand-edited |
| `kotlin-agent-skills.md` | When to invoke the vendored JetBrains Kotlin skills |
...
```
tech-agency/
├── VERSION                      # Single source of truth for the release line
├── .claude-plugin/
│   └── marketplace.json         # Marketplace manifest (makes this repo a plugin source)
├── .claude/                     # ← the plugin itself (marketplace `source`)
│   ├── .claude-plugin/
...
│   │   └── ... (16 more)
│   ├── rules/                   # Coding standards, grouped by domain (auto-loaded)
│   │   ├── shared/              # Cross-cutting: standards, handoffs, board, worktrees
│   │   ├── mobile/              # android/, ios/, shared/ (KMP)
│   │   ├── backend/             # nodejs/, python/, jvm/, kotlin/
│   │   └── web/
│   ├── skills/                  # Slash-command skills (first-party + vendored)
│   │   ├── kick-off/
│   │   ├── ship-it/
│   │   ├── address-feedback/
│   │   ├── daily-sync/
...
├── scripts/
│   ├── sync-version.sh          # VERSION → manifests (--check in CI)
│   └── generate-inventory.sh    # directory → README inventory (--check in CI)
├── docs/
