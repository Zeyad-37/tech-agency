...
|---------|-------|-------------|----------|---------------|
| T-027.13 | @Claude | `/migrate-board` issue fields: titles capped at 256, `agent:@Name` / `priority:Pn` labels normalised, placeholder epics for stories with no epic row, `--check` refuses duplicate live IDs (found by the Steady dry run) | @Zeyad | 2026-09-15 |
